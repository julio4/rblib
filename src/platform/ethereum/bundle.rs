...
/// A bundle of ethereum transactions that adheres to the [Flashbots
/// specification].
///
/// [Flashbots specification]: https://docs.flashbots.net/flashbots-auction/searchers/advanced/rpc-endpoint#eth_sendbundle
#[derive(Debug, Clone, Default)]
pub struct EthereumBundle {
	inner: EthSendBundle,
	recovered: Vec<Recovered<types::Transaction<Ethereum>>>,
}
...
impl EthereumBundle {
	...
	pub fn with_transaction(
		mut self,
		tx: Recovered<types::Transaction<Ethereum>>,
	) -> Self {
...
impl Bundle<Ethereum> for EthereumBundle {
	...
	fn is_eligible(&self, block: &BlockContext<Ethereum>) -> Eligibility {
...
impl Serialize for EthereumBundle {
...
impl<'de> Deserialize<'de> for EthereumBundle {
...
impl DerefMut for EthereumBundle {
	fn deref_mut(&mut self) -> &mut Self::Target {
		&mut self.inner
	}
}

impl TryFrom<EthSendBundle> for EthereumBundle {
	...
	fn try_from(inner: EthSendBundle) -> Result<Self, Self::Error> {
		let txs = inner
			.txs
			.iter()
			.map(|tx| types::Transaction::<Ethereum>::decode_2718(&mut &tx[..]))
